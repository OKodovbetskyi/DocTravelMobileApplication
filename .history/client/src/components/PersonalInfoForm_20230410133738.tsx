...
const PersonalInfoForm = ({personalInfo, ticket, type}) => {
    ...
    const validate = {
        isValid:{
            ...
            phonenumber: (phonenumber)=> phonenumber.lenght>0,
    ...
    const isValidRecord = () =>{
        ...
        Object.keys(personal).forEach((key)=>{
            console.log(key)
          if (validate.isValid[key](personal[key])){
            errors[key] = null;
          } else {
    ...
    <View>
     ...
     <TextInput
      ...
      onFocus={()=>{
       ...
       <TextInput  style={styles.input}
        onChangeText={(text)=>handleInput(text ,'countryissue')}
        value={personal.countryissue}
      label="Passport country issue" 
      helperText={errors.countryissue}
      onFocus={()=>{
        handleError(null, 'countryissue');
    }}/>
      {
        type==='save' ?
        <Button
