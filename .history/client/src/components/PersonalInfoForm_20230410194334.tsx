...
const PersonalInfoForm = ({personalInfo, ticket, type,id}) => {
    const dispatch =useDispatch();
    const fullstate = useSelector(state=>state.bookingUserInformationSlice);
    let personal = useSelector(state=>state.bookingUserInformationSlice.personalinfo)
    ...
    const updateHandler = () =>{
        if (isValidRecord(personal)){
            axios.put(`${URL}${bookingEndp}/${id}`, fullstate)
            .then((response) => {
              console.log('POST request successful:', response.data);
              dispatch(bookingUserInformationActions.clearData);
            })
   ...
   useEffect(()=>{
    dispatch(bookingUserInformationActions.setInitialState(personalInfo))},[personalInfo])
  return (
