...
const IP_ADDRESS = Constants.manifest.extra.IP_ADDRESS;
...
const FlightSearch : React.FC<Props> = () => {
  console.log(`API URL is ${IP_ADDRESS}`);
 ...
 useEffect(() => {
    ...
    axios.get(`http://${IP_ADDRESS}:5002/api/airports`, {
      params:{
        keyword: searchValue
      }
    })
    ...
    const sendData = async () => {
      try {
        const response = await axios.post(
          `http://${IP_ADDRESS}/api/flight-query`,
          flightquery,
