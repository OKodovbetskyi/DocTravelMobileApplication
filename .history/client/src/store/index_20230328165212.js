...
const persistantStorageSlice = createSlice({
    ...
    initialState: initialState.savedTickets,
    reducers:{
        save(state, actions){
            if (actions.payload !== null){
        ...
        remove(state, actions){
            if (state.length>0){
                const filtered = state.filter(ticket=>ticket.itineraries[0].segments[0].id !== actions.payload);
                state = filtered;
                console.log('state len after remove', state.length)
            }
        }
    }
})
