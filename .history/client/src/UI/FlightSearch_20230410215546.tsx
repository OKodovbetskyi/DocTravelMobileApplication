...
import { bookingUserInformationActions, flightqueActions } from '../store';
...
export const FlightSearch : React.FC<Props> = ({navigation}) => {
  ...
  const personaldata = useSelector(state=>state.bookingUserInformationSlice)
  ...
  />
    ...
    <Text style={styles.title}>Flights to {flightquery.destination != null ? flightquery.destination :"" }</Text>
    {
      ...
      <FlatList
        ...
        renderItem={({item}) => 
        <Flights ticket={item} navigation={navigation}/>}
      />
    }
